    ...
    >
      ...
      <div className="max-w-3xl mx-auto px-6 text-center space-y-5 -mt-2 mb-12 md:mb-16">
        <p className="text-base md:text-lg text-muted-foreground leading-relaxed">
          {tr ? (
            <>Her ay <Link to="/calculators/stack-sats" className="text-primary hover:underline">sat biriktiriyor</Link> ya da zaten önemli bir Bitcoin pozisyonu tutuyor olun, bu bitcoin emeklilik hesaplayıcısı finansal bağımsızlık yolculuğunuzu modellemenize yardımcı olur. Mevcut DCA stratejinizin nereye götürdüğünü öngörmek için <strong className="text-foreground">Öngörüleyici</strong>'yi, hedef emeklilik geliriniz için gereken aylık yatırımı tersine hesaplamak için <strong className="text-foreground">Hedef Planlayıcı</strong>'yı ya da Bitcoin varlıklarınızın yıllık harcamalarınızı ne zaman karşılayabileceğini öğrenmek için <strong className="text-foreground">FIRE Modu</strong>'nu kullanın.</>
          ) : (
            <>Whether you're <Link to="/calculators/stack-sats" className="text-primary hover:underline">stacking sats</Link> every month or already holding a significant Bitcoin position, this bitcoin retirement calculator helps you model your journey to financial independence. Use the <strong className="text-foreground">Forecaster</strong> to project where your current DCA strategy leads, the <strong className="text-foreground">Goal Planner</strong> to reverse-engineer the monthly investment needed for your target retirement income, or <strong className="text-foreground">FIRE Mode</strong> to find out when your Bitcoin holdings could cover your annual expenses.</>
          )}
        </p>
        <p className="text-base md:text-lg text-muted-foreground leading-relaxed">
          {tr
