...
interface RetirementThreeModesProps { onSelectMode: (mode: 'forecaster' | 'planner' | 'fire') => void; }

export const RetirementThreeModes = ({ onSelectMode }: RetirementThreeModesProps) => {
  const { language } = useLanguage();
  const tr = language==='tr';
  const modes = [
    { id: 'forecaster' as const, icon: PiggyBank, title: tr?'Tahminci':'Forecaster', subtitle: tr?'Mevcut planınızın sizi götürdüğü yeri görün':'See where your current plan leads', description: tr?'Aylık DMA ile zaten birikim yapıyor musunuz? Tahminci, mevcut varlıklarınız, katkılarınız ve büyüme varsayımlarınıza göre emeklilikteki portföy değerini projelendirir.':'Already stacking sats with a monthly DCA? The Forecaster projects your portfolio value at retirement based on your current holdings, contributions, and growth assumptions.' },
