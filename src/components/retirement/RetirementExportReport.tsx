      ...
      reportElement.innerHTML = `
        <div class="space-y-8">
          <div class="text-center border-b border-border pb-6">
            <h1 class="text-3xl font-bold mb-2 text-foreground">${tr?'Bitcoin Emeklilik Tahmin Raporu':'Bitcoin Retirement Forecast Report'}</h1>
            <p class="text-muted-foreground">${tr?'Oluşturulma tarihi':'Generated on'} ${format(new Date(), 'MMMM d, yyyy')}</p>
          </div>

          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-3">
              <h2 class="text-xl font-semibold text-foreground">${tr?'Emeklilik Planı':'Retirement Plan'}</h2>
            ...
            <div class="space-y-3">
              <h2 class="text-xl font-semibold text-foreground">${tr?'Yatırım Detayları':'Investment Details'}</h2>
              <div class="space-y-2 text-sm">
          ...
          <div class="grid grid-cols-3 gap-4 py-6 border-t border-b border-border">
            <div class="text-center">
              <div class="text-2xl font-bold text-orange-600">${projections.length}</div>
              <div class="text-sm text-muted-foreground">${tr?'Emeklilik yılı':'Years of Retirement'}</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-success">
                ${formatCurrency(projections[0]?.annualBudget || 0, inputs.currency)}
              </div>
              <div class="text-sm text-muted-foreground">${tr?'Yıllık bütçe (1. yıl)':'Annual Budget (Year 1)'}</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-blue-600">${inputs.expectedGrowthRate}%</div>
              <div class="text-sm text-muted-foreground">${tr?'Büyüme varsayımı':'Growth Assumption'}</div>
            </div>
          </div>

          <div class="text-center text-xs text-muted-foreground pt-6">
  ...
  const generatePlannerPNGReport = async () => {
    ...
    try {
      ...
      reportElement.innerHTML = `
        <div class="space-y-8">
          <div class="text-center border-b border-border pb-6">
            <h1 class="text-3xl font-bold mb-2 text-foreground">${tr?'Bitcoin Emeklilik Hedef Planlayıcı Raporu':'Bitcoin Retirement Goal Planner Report'}</h1>
            <p class="text-muted-foreground">${tr?'Oluşturulma tarihi':'Generated on'} ${format(new Date(), 'MMMM d, yyyy')}</p>
          </div>

          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-3">
              <h2 class="text-xl font-semibold text-foreground">${tr?'Emeklilik Hedefi':'Retirement Goal'}</h2>
              <div class="space-y-2 text-sm">
            ...
            <div class="space-y-3">
              <h2 class="text-xl font-semibold text-foreground">${tr?'Yatırım Gereksinimleri':'Investment Requirements'}</h2>
              <div class="space-y-2 text-sm">
          ...
          <div class="grid grid-cols-3 gap-4 py-6 border-t border-b border-border">
            <div class="text-center">
              ...
              <div class="text-sm text-muted-foreground">${tr?'Hedef değerlendirmesi':'Goal Assessment'}</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-blue-600">
                ${formatCurrency(goalResults.requiredMonthlyInvestment, goalInputs.currency)}
              </div>
              <div class="text-sm text-muted-foreground">${tr?'Aylık yatırım':'Monthly Investment'}</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-orange-600">${yearsToRetirement}</div>
              <div class="text-sm text-muted-foreground">${tr?'Hedefe kalan yıl':'Years to Goal'}</div>
            </div>
          </div>
