    ...
    <>
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <div className="absolute -left-[9999px]" aria-hidden="true">
          <div ref={reportRef} className="w-[800px] p-8 bg-[#1a1a2e] text-white font-sans">
            <h2 className="text-h2 font-bold mb-1">{tr ? 'Bitcoin Yatırım Projeksiyonu' : 'Bitcoin Investment Projection'}</h2>
            <p className="text-sm text-gray-400 mb-6">{tr ? 'Oluşturuldu:' : 'Generated on'} {new Date().toLocaleDateString()} — bitcoincalculator.tools</p>

            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-xs text-gray-400">{tr ? 'Başlangıç Yatırımı' : 'Initial Investment'}</p>
                <p className="text-lg font-bold">{formatCurrency(lumpSum)}</p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-xs text-gray-400">{tr ? 'Aylık DCA' : 'Monthly DCA'}</p>
                <p className="text-lg font-bold">{formatCurrency(monthlyContribution)}</p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-xs text-gray-400">{tr ? 'Zaman Ufku' : 'Time Horizon'}</p>
                <p className="text-lg font-bold">{timeHorizon} {tr ? 'yıl' : 'years'}</p>
              </div>
            </div>

            <div className="bg-white/5 rounded-lg p-4 mb-6">
              <p className="text-xs text-gray-400 mb-1">{tr ? 'Hesaplama Anındaki BTC Fiyatı' : 'BTC Price at Calculation'}</p>
              <p className="text-lg font-bold">${btcPrice.toLocaleString()}</p>
            </div>

            {results.map((r) => (
              <div key={r.modelId} className="bg-white/5 rounded-lg p-4 mb-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <span className="text-gray-400">{tr ? 'Tahmini Değer: ' : 'Projected Value: '}</span>
                    <span className="font-mono">{formatCurrency(r.finalValue)}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">{tr ? 'Kâr: ' : 'Profit: '}</span>
                  ...
                  <div>
                    <span className="text-gray-400">{tr ? 'Tahmini BTC: ' : 'Est. BTC: '}</span>
