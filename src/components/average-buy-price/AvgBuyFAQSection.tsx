...
const faqsEn = [
  ...
  { q: 'How do I track scattered Bitcoin purchases across exchanges?', a: 'Download the annual transaction CSV from each exchange (Coinbase → Reports, Binance → Wallet history, Kraken → Ledgers) plus any wallet export like Exodus or Electrum. Consolidate every BUY row into a single spreadsheet with columns for date, BTC amount, USD spent, and fee. Paste each lot into the calculator above — the weighted average buy price and total cost basis update on every row, so you finish with one canonical number instead of 3-5 disconnected exchange dashboards.' },
  { q: 'How do I figure out Bitcoin capital gains without records?', a: 'When historical CSVs are missing, the IRS accepts a "reasonable reconstruction." Pull bank and card statements for every fiat deposit sent to an exchange, then match each deposit to the daily BTC closing price using our Bitcoin Converter (historical mode). Enter each reconstructed lot in the calculator above to get a defensible cost basis. If a price cannot be pinpointed, use the highest plausible price for that day — it raises your basis, lowers your gain, and keeps you conservative if the IRS reviews the return.' },
];
...
const faqsTr = [
  ...
  { q: 'Farklı borsalara dağılmış Bitcoin alımlarını nasıl takip ederim?', a: 'Her borsanın yıllık işlem CSV\'sini indirin (Coinbase → Raporlar, Binance → Cüzdan geçmişi, Kraken → Ledgers) ve Exodus veya Electrum gibi cüzdan dışa aktarımlarını da ekleyin. Tüm ALIM satırlarını tarih, BTC miktarı, harcanan USD ve ücret sütunlarıyla tek bir tabloda birleştirin. Her lotu yukarıdaki hesap makinesine yapıştırın; ağırlıklı ortalama alış fiyatı ve toplam maliyet bazı her satırda güncellenir, böylece 3-5 dağınık borsa panosu yerine tek bir kanonik rakamla bitirirsiniz.' },
  { q: 'Kayıt olmadan Bitcoin sermaye kazançlarını nasıl hesaplarım?', a: 'Tarihsel CSV\'ler eksikse, IRS "makul yeniden yapılandırma" kabul eder. Bir borsaya gönderilen her fiat mevduatı için banka ve kart ekstrelerini çıkarın, sonra her mevduatı Bitcoin Dönüştürücümüzü (tarihsel mod) kullanarak günlük BTC kapanış fiyatıyla eşleştirin. Yeniden yapılandırılan her lotu yukarıdaki hesap makinesine girerek savunulabilir bir maliyet bazı elde edin. Bir fiyat kesin belirlenemezse, o günün makul en yüksek fiyatını kullanın — bu, bazınızı yükseltir, kazancınızı düşürür ve IRS incelemesi durumunda sizi muhafazakâr tutar.' },
];
