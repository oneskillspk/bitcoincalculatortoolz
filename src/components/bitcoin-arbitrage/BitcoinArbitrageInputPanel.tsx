    ...
    <Card className="glass-morphism-card border-border/20 shadow-sm">
      <CardContent className="p-6 space-y-6">
        ...
        <div className="space-y-4 p-4 rounded-lg bg-muted/30 border border-border/30">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 [&>*:nth-child(3)]:col-span-2 sm:[&>*:nth-child(3)]:col-span-1">
            {(Object.entries(FEE_PRESETS) as [FeePresetKey, typeof FEE_PRESETS[FeePresetKey]][]).map(([key, preset]) => (
              <Button key={key} type="button" variant={p.feePreset === key ? 'default' : 'outline'} onClick={() => p.applyFeePreset(key)} className="h-16 min-w-0 max-w-full flex-col items-start justify-center gap-1 overflow-hidden px-3 py-2 text-left whitespace-normal">
                <span className="block w-full truncate text-sm font-semibold leading-tight">{preset.label}</span>
                <span className="block w-full truncate text-[11px] leading-tight opacity-80">Taker {preset.taker}% · Slip {preset.slippage}%</span>
              </Button>
            ))}
          </div>
