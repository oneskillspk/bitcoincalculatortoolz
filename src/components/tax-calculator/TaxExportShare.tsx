...
export const TaxExportShare: React.FC<TaxExportShareProps> = ({ results, config }) => {
  ...
  const exportCSV = () => {
    exportCsv({
      ...
      rows: [
        [tr ? 'Kısa vadeli kazanç (USD)' : 'Short-term gains (USD)', csvNumber(results.federalTax.shortTermGains)],
        [tr ? 'Uzun vadeli kazanç (USD)' : 'Long-term gains (USD)', csvNumber(results.federalTax.longTermGains)],
        [tr ? 'Net sermaye kazancı (USD)' : 'Net capital gains (USD)', csvNumber(results.federalTax.netCapitalGains)],
        [tr ? 'Federal vergi (USD)' : 'Federal tax (USD)', csvNumber(results.federalTax.totalTaxOwed)],
        [tr ? 'Eyalet vergisi (USD)' : 'State tax (USD)', csvNumber(results.stateTax?.totalTaxOwed ?? 0)],
